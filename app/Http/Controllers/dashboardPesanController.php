...
class dashboardPesanController extends Controller
{
    ...
    {
        $thnbln = DB::table('rkppesanheadx')->selectraw("DISTINCT DATE_FORMAT(tanggal, '%Y%m') as thnbln, DATE_FORMAT(tanggal, '%M %Y') as thnbln2")->orderbyraw('1 desc')->get();
        return view('dashboardpesan', compact('thnbln'));
    }
    ...
    public function store(Request $request)
    {
        ...
        switch($request->mode){
            case 'getdasboardpesan':
                $lastupdate = DB::table('rkppesanheadx')->selectraw("DATE_FORMAT(max(dateukehosting), '%d-%m-%Y %H:%i') as tanggal")->first();
                ...
                if ($g == 'na' && $s == 'na') {
                    $que = "CALL spgraphpesanh('$thnbln')";
                    $data = DB::select(DB::raw($que));
                    $htm = $this->createtable($data);
                    $graphx = array_column($data, 'x');
                    $graphy = array_column($data, 'y'); 

                    $gudang = DB::table('vwpesanh_bulanan_gudang')->where('thnbln', $thnbln)->selectraw("kdgudang as kode, total")->orderby('total', 'desc')->get();
                    $htmlgudang = $this->createheaderbutton($gudang, 'gudang');

                    $salesman = DB::table('vwpesanh_bulanan_salesman')->where('thnbln', $thnbln)->selectraw("namasalesman as kode, total")->orderby('total', 'desc')->get();
                    ...
                    if ($g != 'na' && $s = 'na') {
                        $que = "CALL spgraphpesanh_pergudang('$thnbln', '$g')";
                        $mode = 2;
                    }
                    else{
                        $que = "CALL spgraphpesanh_persalesman('$thnbln', '$s')";
                        $mode = 3;
                    }
